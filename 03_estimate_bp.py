...
def estimate_bp(feature_array, bp_array, num_experiments, camera_type):
    """
    血圧推定を行う．
    ...
    # 特徴量カウント用リスト
    selected_features_sbp_count_list = [0] * feature_array.shape[2]
    selected_features_dbp_count_list = [0] * feature_array.shape[2]
    ...
    for test_idx in range(num_experiments):
        ...
        if np.isnan(X_train).all():
            ...
            mean_val_X_train = np.nanmean(X_train)
            X_train = np.nan_to_num(X_train, nan=mean_val_X_train)

        if np.isnan(X_test).all():
            X_test = np.nan_to_num(X_test, nan=0.5)
        else:
            mean_val_X_test = np.nanmean(X_test)
            X_test = np.nan_to_num(X_test, nan=mean_val_X_test)
...
def main():
    dir_load_name = "E:\\2024_ex\\data_all_20241212\\"
    ...
    for subject_idx, filepath in enumerate(filepaths):
        ...
        for state_cnt_idx, state_cnt_append in enumerate(state_cnt_append_list):
    ...
    if filter_flag:      
        ...
        mae_array[2], sbp_predicted, dbp_predicted = estimate_bp(filtered_features_rgb_nir, filtered_bp_array, filtered_num_experiments, "RGB-NIR G&NIR")
        print()
    else:
        print("RGB-NIR camera (only G)")
        #mae_array[0] = estimate_bp(feature_array_rgb_nir_g, bp_array, num_experiments)
        mae_array[0], sbp_predicted_g, dbp_predicted_g = estimate_bp(feature_array_rgb_nir_g, bp_array, num_experiments, "RGB-NIR G")
        print()
    
        print("RGB-NIR camera (only NIR)")
        #mae_array[1] = estimate_bp(feature_array_rgb_nir_nir, bp_array, num_experiments)
        mae_array[1], sbp_predicted_nir, dbp_predicted_nir = estimate_bp(feature_array_rgb_nir_nir, bp_array, num_experiments, "RGB-NIR NIR")
        print()
        ...
        print("RGB-NIR camera (G & NIR)")
        #mae_array[2] = estimate_bp(feature_array_rgb_nir, bp_array, num_experiments)
        mae_array[2], sbp_predicted_gnir, dbp_predicted_gnir = estimate_bp(feature_array_rgb_nir, bp_array, num_experiments, "RGB-NIR G&NIR")
        print()
